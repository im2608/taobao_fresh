...
def createTrainingSet(window_start_date, window_end_date, nag_per_pos, samples, cal_feature_importance):
    ...
    feature_mat, feature_cnt = feature_days_from_1st_last_behavior_item(window_start_date, window_end_date, samples, cal_feature_importance, total_feature_cnt)
    ...
    time_start = time.clock()
    feature_mat, feature_cnt = feature_how_many_users_behavior_item(window_start_date, window_end_date, samples, cal_feature_importance, total_feature_cnt)
    total_feature_cnt += addSubFeatureMatIntoFeatureMat(feature_mat, feature_cnt, Xmat, total_feature_cnt)
    time_end = time.clock()
    ...
    time_start = time.clock()
    feature_mat, feature_cnt = feature_item_sals_volume(window_start_date, window_end_date, samples, cal_feature_importance, total_feature_cnt)
    total_feature_cnt += addSubFeatureMatIntoFeatureMat(feature_mat, feature_cnt, Xmat, total_feature_cnt)
    time_end = time.clock()
    ...
    time_start = time.clock()
    feature_mat, feature_cnt = feature_behavior_on_date(BEHAVIOR_TYPE_FAV, verify_date, samples, cal_feature_importance, total_feature_cnt)
    total_feature_cnt += addSubFeatureMatIntoFeatureMat(feature_mat, feature_cnt, Xmat, total_feature_cnt)
    time_end = time.clock()
    ...
    time_start = time.clock()
    feature_mat, feature_cnt = feature_behavior_on_date(BEHAVIOR_TYPE_CART, verify_date, samples, cal_feature_importance, total_feature_cnt)
    total_feature_cnt += addSubFeatureMatIntoFeatureMat(feature_mat, feature_cnt, Xmat, total_feature_cnt)
    time_end = time.clock()
    ...
    for pre_days in pre_days_list:
        ...
        time_start = time.clock()
        feature_mat, feature_cnt = feature_user_item_behavior_ratio(window_end_date, pre_days, samples, cal_feature_importance, total_feature_cnt)
        total_feature_cnt += addSubFeatureMatIntoFeatureMat(feature_mat, feature_cnt, Xmat, total_feature_cnt)
        time_end = time.clock()
    ...
    time_start = time.clock()
    feature_mat, feature_cnt = feature_behavior_cnt_before_1st_buy(window_start_date, window_end_date, samples, cal_feature_importance, total_feature_cnt)
    total_feature_cnt += addSubFeatureMatIntoFeatureMat(feature_mat, feature_cnt, Xmat, total_feature_cnt)
    time_end = time.clock()
    ...
    time_start = time.clock()
    feature_mat, feature_cnt = feature_last_opt_item(window_start_date, window_end_date, samples, cal_feature_importance, total_feature_cnt)
    total_feature_cnt += addSubFeatureMatIntoFeatureMat(feature_mat, feature_cnt, Xmat, total_feature_cnt)
    time_end = time.clock()
    ...
    feature_mat, feature_cnt = feature_mean_days_between_buy_user_item(window_start_date, window_end_date, samples, cal_feature_importance, total_feature_cnt)
    total_feature_cnt += addSubFeatureMatIntoFeatureMat(feature_mat, feature_cnt, Xmat, total_feature_cnt)
    time_end = time.clock()
    ...
    time_start = time.clock()
    feature_mat, feature_cnt = feature_buy_view_ratio(window_start_date, window_end_date, samples, cal_feature_importance, total_feature_cnt)
    total_feature_cnt += addSubFeatureMatIntoFeatureMat(feature_mat, feature_cnt, Xmat, total_feature_cnt)
    time_end = time.clock()
    ...
    time_start = time.clock()
    feature_mat, feature_cnt = feature_last_opt_category(window_start_date, window_end_date, samples, cal_feature_importance, total_feature_cnt)
    total_feature_cnt += addSubFeatureMatIntoFeatureMat(feature_mat, feature_cnt, Xmat, total_feature_cnt)
    time_end = time.clock()
    ...
    time_start = time.clock()
    feature_mat, feature_cnt = feature_how_many_buy_category(window_start_date, window_end_date, samples, cal_feature_importance, total_feature_cnt)
    total_feature_cnt += addSubFeatureMatIntoFeatureMat(feature_mat, feature_cnt, Xmat, total_feature_cnt)
    time_end = time.clock()
    ...
    time_start = time.clock()
    feature_mat, feature_cnt = feature_mean_days_between_buy_user_category(window_start_date, window_end_date, samples, cal_feature_importance, total_feature_cnt)
    total_feature_cnt += addSubFeatureMatIntoFeatureMat(feature_mat, feature_cnt, Xmat, total_feature_cnt)
    time_end = time.clock()
    ...
    for i, pre_days in enumerate(pre_days_list):
        feature_mat, feature_cnt = feature_user_behavior_cnt_on_category(pre_days, window_end_date, samples, user_behavior_cnt_on_item[i], cal_feature_importance, total_feature_cnt)
        total_feature_cnt += addSubFeatureMatIntoFeatureMat(feature_mat, feature_cnt, Xmat, total_feature_cnt)
    time_end = time.clock()
    ...
    time_start = time.clock()
    for i, pre_days in enumerate(pre_days_list):
        feature_mat, feature_cnt = feature_how_many_behavior_user(pre_days, window_end_date, samples, cal_feature_importance, total_feature_cnt)
        total_feature_cnt += addSubFeatureMatIntoFeatureMat(feature_mat, feature_cnt, Xmat, total_feature_cnt)
    time_end = time.clock()
    ...
    time_start = time.clock()
    feature_mat, feature_cnt = feature_mean_days_between_buy_user(window_start_date, window_end_date, samples, cal_feature_importance, total_feature_cnt)
    total_feature_cnt += addSubFeatureMatIntoFeatureMat(feature_mat, feature_cnt, Xmat, total_feature_cnt)
    time_end = time.clock()
    ...
    time_start = time.clock()
    feature_mat, feature_cnt = feature_last_behavior_user(window_start_date, window_end_date, samples, cal_feature_importance, total_feature_cnt)
    total_feature_cnt += addSubFeatureMatIntoFeatureMat(feature_mat, feature_cnt, Xmat, total_feature_cnt)
    time_end = time.clock()
    ...
    time_start = time.clock()
    feature_mat, feature_cnt = feature_how_many_days_for_behavior(window_start_date, window_end_date, samples, cal_feature_importance, total_feature_cnt)
    total_feature_cnt += addSubFeatureMatIntoFeatureMat(feature_mat, feature_cnt, Xmat, total_feature_cnt)
    time_end = time.clock()
    ...
    time_start = time.clock()
    feature_mat, feature_cnt = feature_how_many_buy_in_days(window_start_date, window_end_date, samples, cal_feature_importance, total_feature_cnt)
    total_feature_cnt += addSubFeatureMatIntoFeatureMat(feature_mat, feature_cnt, Xmat, total_feature_cnt)
    time_end = time.clock()
    ...
    time_start = time.clock()
    feature_mat, feature_cnt = feature_days_from_1st_last_behavior_category(window_start_date, window_end_date, samples, cal_feature_importance, total_feature_cnt)
    total_feature_cnt += addSubFeatureMatIntoFeatureMat(feature_mat, feature_cnt, Xmat, total_feature_cnt)
    time_end = time.clock()
    ...
    time_start = time.clock()
    for i, pre_days in enumerate(pre_days_list):
        feature_mat, feature_cnt = feature_beahvior_cnt_on_category(pre_days, window_end_date, samples, cal_feature_importance, total_feature_cnt)
        total_feature_cnt += addSubFeatureMatIntoFeatureMat(feature_mat, feature_cnt, Xmat, total_feature_cnt)
    ...
    time_start = time.clock()
    feature_mat, feature_cnt = feature_how_many_users_bought_category(window_start_date, window_end_date, samples, cal_feature_importance, total_feature_cnt)
    total_feature_cnt += addSubFeatureMatIntoFeatureMat(feature_mat, feature_cnt, Xmat, total_feature_cnt)
    time_end = time.clock()
    ...
    feature_mat, feature_cnt = feature_category_sals_volume(window_start_date, window_end_date, samples, cal_feature_importance, total_feature_cnt)
    total_feature_cnt += addSubFeatureMatIntoFeatureMat(feature_mat, feature_cnt, Xmat, total_feature_cnt)
    time_end = time.clock()
    ...
    feature_mat, feature_cnt = feature_sales_ratio_itme_category(item_sales_vol, category_sales_vol, cal_feature_importance, total_feature_cnt)
    total_feature_cnt += addSubFeatureMatIntoFeatureMat(feature_mat, feature_cnt, Xmat, total_feature_cnt)
    ...
    feature_mat, feature_cnt = feature_buyer_ratio_item_category(user_cnt_buy_item, user_cnt_buy_category, cal_feature_importance, total_feature_cnt)
    total_feature_cnt += addSubFeatureMatIntoFeatureMat(feature_mat, feature_cnt, Xmat, total_feature_cnt)
    ...
    for i, pre_days in enumerate(pre_days_list):
        ...
        feature_mat, feature_cnt = feature_beahvior_cnt_on_item(pre_days, window_end_date, user_behavior_cnt_on_item[i],
                                                                samples, cal_feature_importance, total_feature_cnt)
        total_feature_cnt += addSubFeatureMatIntoFeatureMat(feature_mat, feature_cnt, Xmat, total_feature_cnt)
    time_end = time.clock()
...
# 训练得到特征的 importance
def GBDT_slideWindows(window_start_date, window_end_date, cal_feature_importance, n_estimators):
    nag_per_pos = 10
