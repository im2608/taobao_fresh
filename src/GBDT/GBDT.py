...
def createTrainingSet(window_start_date, window_end_date, nag_per_pos, samples):
    ...
    time_start = time.clock()
    feature_mat = feature_last_opt_item(window_start_date, window_end_date, samples)
    Xmat = np.column_stack((Xmat, feature_mat))
    time_end = time.clock()
    print("        %s feature_last_opt_item takes %d seconds   \r" % (getCurrentTime(), time_end - time_start), end="")
...
# 训练得到特征的 importance
def GBDT_slideWindows(window_start_date, window_end_date, nag_per_pos, n_estimators, max_depth, start_from, user_cnt):
    print("%s slide windows from %s to %s, start from %d, user count %d   \r" % 
    ...
    samples, Ymat = takeSamples(window_start_date, window_end_date, nag_per_pos, False)

    Xmat = createTrainingSet(window_start_date, window_end_date, nag_per_pos, samples)
    if (len(samples) == 0):
